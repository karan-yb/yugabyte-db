...
public class CdcStreamManager {
  ...
  public List<CdcStream> getAllCdcStreams(Universe universe) throws Exception {
    try (YBClient client = getYBClientForUniverse(universe)) {
      ...
      ListCDCStreamsResponse response = client.listCDCStreams(null, null, MasterReplicationOuterClass.IdTypePB.NAMESPACE_ID);

      LOG.info("Got response for 'listCDCStreams' for universeId='{}': hasError='{}', size='{}'", universe.universeUUID, response.hasError(), response.getStreams() != null ? response.getStreams().size() : -1);
      ...
      for (CDCStreamInfo streamInfo : response.getStreams()) {
        ...
        CdcStream stream = new CdcStream(streamInfo.getStreamId(), options, streamInfo.getNamespaceId());
        streams.add(stream);
      }
  ...
  private YBTable getFirstTable(YBClient client, String databaseName) throws Exception {
    ...
    if (tid.equals("")) {
      throw new Exception("Must have at least a table in database.");
    }

    return client.openTableByUUID(tid);
  }

  public CdcStreamCreateResponse createCdcStream(Universe universe, String databaseName) throws Exception {
    return createCdcStream(universe, databaseName, "PROTO", "IMPLICIT");
  }

  public CdcStreamCreateResponse createCdcStream(Universe universe, String databaseName, String format, String checkpointType) throws Exception {
    try (YBClient client = getYBClientForUniverse(universe)) {

      LOG.info("Creating CDC stream for universeId='{}' dbName='{}' format='{}', checkpointType='{}'", universe.universeUUID, databaseName, format, checkpointType);

      // TODO: file bug to NOT require table reference when creating a CDC stream (ybadmin does not require)
      YBTable table = getFirstTable(client, databaseName);

      CreateCDCStreamResponse response = client.createCDCStream(table, databaseName, format, checkpointType);

      CdcStreamCreateResponse result = new CdcStreamCreateResponse(response.getStreamId());
      LOG.info("Created CDC stream id='{}' for universeId='{}' dbName='{}' format='{}', checkpointType='{}'", result.getStreamId(), universe.universeUUID, databaseName, format, checkpointType);
      return result;
    }
  }

  public CdcStreamDeleteResponse deleteCdcStream(Universe universe, String streamId) throws Exception {
    try (YBClient client = getYBClientForUniverse(universe)) {
      HashSet<String> streamsToDelete = new HashSet<>();
      streamsToDelete.add(streamId);

      DeleteCDCStreamResponse response = client.deleteCDCStream(streamsToDelete, true /*ignoreErrors*/, true /*forceDelete*/);
