...
public class CdcStreamManagerTest {
  ...
  private YBClientService mockClientService;
  private CdcStreamManager streamManager;

  @Before
  public void setup() {
    mockClient = mock(YBClient.class);
    mockClientService = mock(YBClientService.class);
  ...
  public void getAllCdcStreams_valid() throws Exception {
    ...
    when(response.getStreams()).thenReturn(new ArrayList<CDCStreamInfo>() {{ add(mockStream); }});
  ...
  public void getAllCdcStreams_error() throws Exception {
    ...
    assertThrows("err", Exception.class, () -> streamManager.getAllCdcStreams(mock(Universe.class)));
  }
}
